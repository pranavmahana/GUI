...
class VibrationAnalyzer:
    ...
    def plot_glevels(self):
        if self.data is not None and self.velocity_data is not None:
            ...
            for i in range(num_plots):
                ...
                # Plot velocity data
                velocity_profile = self.velocity_data.iloc[:, 1]  # Use velocity profile from the first channel
                ax.plot(velocity_profile, label="Velocity", color='orange')
    ...
    def zoom_glevel_plot(self):
        if self.glevel_plot_index is not None:
            ...
            plt.plot(self.velocity_data.iloc[:, 1], label="Velocity", color='orange')  # Use velocity profile from the first channel
