...
class VibrationAnalyzer:
    ...
    def plot_glevels(self):
        if self.data is not None:
            ...
            for i in range(num_plots):
                ...
                ax.set_ylabel("G-levels logarithmic", fontsize=8)  # Label y-axis
    ...
    def zoom_glevel_plot(self):
        if self.glevel_plot_index is not None:
            ...
            plt.title(channel)
            plt.xlabel("Time (s)")
            plt.ylabel("G-levels logarithmic")
    ...
    def plot_psd(self):
        if self.data is not None and self.velocity_data is not None:
            ...
            for i in range(num_plots):
                ...
                # Calculate PSD using Welch method for main data
                f, p_s_d = welch(self.data.iloc[:, i + 1], fs=self.sampling_frequency.get())

                ax.semilogy(f, p_s_d, label="PSD - Main Data")

                # Plot velocity data
                ax.plot(self.velocity_data.iloc[:, 0], self.velocity_data.iloc[:, i + 1], label="Velocity")
